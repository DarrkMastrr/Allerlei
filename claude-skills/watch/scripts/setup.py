...
#
# Priority: Groq > Replicate > OpenAI (first key found wins).
# Override per-run with --whisper groq|replicate|openai.
#
# Groq (preferred): whisper-large-v3, cheapest and fastest.
#   https://console.groq.com/keys
#
# Replicate: openai/whisper large-v3 on Replicate infrastructure.
#   https://replicate.com/account/api-tokens
#
# OpenAI (fallback): whisper-1.
...
def _have_api_key() -> tuple[bool, str | None]:
    if _read_env_key("GROQ_API_KEY"):
        return True, "groq"
    if _read_env_key("REPLICATE_API_TOKEN"):
...
def cmd_check() -> int:
    ...
    if not s["has_api_key"]:
        parts.append("no Whisper API key (GROQ_API_KEY, REPLICATE_API_TOKEN, or OPENAI_API_KEY)")
...
def cmd_install() -> int:
    ...
    print("")
    print(f"  Edit {CONFIG_FILE} and set any one of:")
    print("    GROQ_API_KEY=...         (preferred — cheapest & fastest; console.groq.com/keys)")
    print("    REPLICATE_API_TOKEN=...  (Replicate openai/whisper; replicate.com/account/api-tokens)")
